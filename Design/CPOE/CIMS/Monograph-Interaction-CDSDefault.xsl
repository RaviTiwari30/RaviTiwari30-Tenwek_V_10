...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mims="http://www.mims.com">
	...
	<xsl:template match="Interaction">
		<xsl:variable name="numDT" select="count(//DuplicateTherapy/Warning/Duplicate)"/>
		...
		<xsl:variable name="numTotal" select="$numDT + $numHealth + $numAllergy + $numInteract + $numDose + $numPreg + $numLact"/>

		<!-- Only count number of drugs that has dose alerts -->
		<xsl:if test="$numTotal &gt; 0">
			...
			<xsl:otherwise>
				<xsl:for-each select="Route">
					...
					<xsl:if test="$Mode='pregnancy' and Pregnancy|WOCBA">
						<xsl:variable name="DivId">pc_<xsl:value-of select="translate($IntProd1, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ','abcdefghijklmnopqrstuvwxyz_')"/>
						</xsl:variable>
			...
			<ul class="NoBulletNoIndent">
				<xsl:for-each select="*">
					<xsl:choose>
						...
						<xsl:otherwise>
							<li>The patient has a known history of allergic reaction to <xsl:value-of select="name()"/>, <xsl:value-of select="@name"/>.</li>
							<xsl:choose>
								<xsl:when test="name()='Molecule' and .//PrescribingMolecule/@name!=@name">
									<li>
										<xsl:value-of select="$IntProd1"/> contains <xsl:value-of select=".//PrescribingMolecule/@name"/>, which belongs to the same substance class as <xsl:value-of select="@name"/>.</li>
								</xsl:when>
								<xsl:when test="name()='Molecule' and .//PrescribingMolecule/@name=@name">
									<li>
										<xsl:value-of select="$IntProd1"/> contains <xsl:value-of select=".//PrescribingMolecule/@name"/>.</li>
								</xsl:when>
								<xsl:when test="name()='SubstanceClass'">
									<li>
										<xsl:value-of select="$IntProd1"/> contains <xsl:value-of select=".//PrescribingMolecule/@name"/>, which belongs to that same substance class.</li>
								</xsl:when>
							</xsl:choose>
						</xsl:otherwise>
					</xsl:choose>
...
Studies in animals have shown evidence of an increased occurrence of fetal damage, the significance of which is considered uncertain in humans.</mims:preg_cat>
   ...
   BODY {COLOR: #6B696B; FONT-FAMILY: Tahoma;FONT-SIZE: 12pt;} 
   TR.clsOdd { background-Color: #dcdcf5; } 
   TR.clsEven { background-color: #dcdcf5; }
   TD.FirstColumn{ background-Color: #dcdcf5;FONT-FAMILY: Tahoma; FONT-SIZE: 12pt;COLOR:black; } 
     TR.Header{ background-Color: #dcdcf5;FONT-FAMILY: Tahoma; FONT-SIZE: 12pt;COLOR:black; } 
     table{FONT-FAMILY:Tahoma; FONT-SIZE:14pt;COLOR: #6B696B;}
		</STYLE>
		<p align="center">
			<strong>
				<xsl:value-of select="@name"/>
		...
		<table width="100%">
			...
			<xsl:if test="TherapeuticClasses/@count &gt;  0">
				<tr>
			...
			<xsl:if test="Packages/@count &gt;  0">
				<tr>
					<td class="FirstColumn">Package</td>
					<td>
						<table width="100%">
							<tr class="Header">
								<td class="FirstColumn" width="70%">Pack</td>
