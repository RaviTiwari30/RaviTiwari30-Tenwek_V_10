...
public partial class Design_CPOE_InfusionMedicationRecord : System.Web.UI.Page
{
        ...
        try
        {
            if (!IsPostBack)
            {
                ...
                if (dtTimes.Rows.Count > 0)
                {
                    ...
                    ddlRoute.DataBind();
                    ddlRoute.SelectedValue = "Intravenours (IV)";
                }
    ...
    protected void BindMedicineGrid()
    {
        try
        {
            ...
            sb.Append("  SELECT t.*,IF(ReceiveQty=0,'Not Issued',IF((ReceiveQty-IFNULL(GivenQty,0))=0,'Running',IF(DATE(duration)<DATE(NOW()),'Completed',IF(IFNULL(a,'0')=0,'Running',IF(a=2,'Stopped','')))))STATUS, ");
            sb.Append(" ROUND((ReceiveQty-IFNULL(GivenQty,0)),0)RemainingQty  FROM (SELECT IFNULL(om.IndentNo,'')IndentNo,");
            sb.Append(" (SELECT IVRate FROM infusion_medicationrecord mr WHERE itemID=om.MedicineID AND TransactionID=pmh.TransactionID AND IFNULL(IndentNo,'')=IFNULL(om.IndentNo,'') ORDER BY id DESC LIMIT 1 )IVRate,");
            sb.Append("  (SELECT STATUS FROM infusion_medicationrecord mr WHERE itemID=om.MedicineID AND TransactionID=pmh.TransactionID AND IFNULL(IndentNo,'')=IFNULL(om.IndentNo,'') ORDER BY id DESC LIMIT 1 )a,");
            sb.Append("  ROUND((SELECT SUM(Qty) FROM infusion_medicationrecord mr WHERE itemID=om.MedicineID AND TransactionID=pmh.TransactionID AND IF(SUBSTRING(om.MedicineID,1,3)='OTH',IndentNo=IndentNo,IndentNo=om.IndentNo) ),0)GivenQty,");
            ...
            sb.Append("  (SELECT CONCAT(title,' ',NAME) FROM employee_master WHERE EmployeeID=om.EntryBy)EName ");
         // sb.Append("  FROM orderset_infusionmedication om INNER JOIN patient_medical_history pmh ON pmh.Transaction_ID = om.TransactionID");
            sb.Append("  FROM orderset_infusionmedication om INNER JOIN patient_medical_history pmh ON pmh.TransactionID = om.TransactionID ");
    ...
    protected void rptMedicine_ItemCommand(object sender, RepeaterCommandEventArgs e)
    {
        try
        {
            ...
            if (opType == "SHOW")
            {
                ...
                DataTable dt = BindMedicineDetail(ID_Date.Split('#')[0], "");
    ...
    protected DataTable BindMedicineDetail(string ItemID, string IndentNo)
    {
        try
        {
            ...
            if (ItemID.Substring(0, 3).ToString().ToUpper() == "OTH")
                dt = StockReports.GetDataTable("SELECT ID,Remark,Round(Qty,0)Qty,IndentNo,ItemID,TransactionID,DATE_FORMAT(DATE,'%d-%b-%Y')DATE,DATE_FORMAT(TIME,'%l:%i %p')TIME,(SELECT CONCAT(title,' ',NAME) FROM Employee_master WHERE EmployeeID=EntryBy)EntryBy,DATE_FORMAT(EntryDateTime,'%d-%b-%Y %l:%i %p')EntryDateTime,Dose,Route,Frequency,StatusType,IVRate,SyringeContraction,SyringeContraction1,If(SyringeChanged=1,'yes','no')SyringeChanged1,DripRate FROM infusion_medicationrecord WHERE TransactionID='" + ViewState["TID"].ToString() + "' and ItemID='" + ItemID + "'  ORDER BY ID DESC");
            else
                dt = StockReports.GetDataTable("SELECT ID,Remark,Round(Qty,0)Qty,IndentNo,ItemID,TransactionID,DATE_FORMAT(DATE,'%d-%b-%Y')DATE,DATE_FORMAT(TIME,'%l:%i %p')TIME,(SELECT CONCAT(title,' ',NAME) FROM Employee_master WHERE EmployeeID=EntryBy)EntryBy,DATE_FORMAT(EntryDateTime,'%d-%b-%Y %l:%i %p')EntryDateTime,Dose,Route,Frequency,StatusType,IVRate,SyringeContraction,SyringeContraction1,If(SyringeChanged=1,'yes','no')SyringeChanged1,DripRate FROM infusion_medicationrecord WHERE TransactionID='" + ViewState["TID"].ToString() + "' AND IndentNo='" + Util.GetString(IndentNo) + "' and ItemID='" + ItemID + "' order by date desc,time desc");
            return dt;
        }
        catch (Exception ex)
        {
    ...
    protected void rptMedicineDetails_ItemCommand(object sender, RepeaterCommandEventArgs e)
    {
        try
        {
            ...
            {
                string qry = "Delete from infusion_medicationrecord where ID=" + e.CommandArgument.ToString().Split('#')[3] + " and TransactionID='" + e.CommandArgument.ToString().Split('#')[2] + "' and ItemID='" + e.CommandArgument.ToString().Split('#')[0] + "' and IndentNo='" + e.CommandArgument.ToString().Split('#')[1] + "'";
            ...
            else
            {
                ...
                TextBox txtIVRate = HeaderTemplate.FindControl("txtIVRate") as TextBox;
                TextBox txtDripRate = HeaderTemplate.FindControl("txtDripRate") as TextBox;
                TextBox txtSyringeContraction = HeaderTemplate.FindControl("txtSyringeContraction") as TextBox;
                DropDownList ddlSyringeContraction = HeaderTemplate.FindControl("ddlSyringeContraction") as DropDownList;
                CheckBox chkSyringeChanged = HeaderTemplate.FindControl("chkSyringeChanged") as CheckBox;
                ...
                decimal IssuedQty = 0;
                if (ItemID.Substring(0, 3).ToString().ToUpper() != "OTH")
                    IssuedQty = Util.GetDecimal(StockReports.ExecuteScalar("Select Sum(ifnull(Qty,0)) from infusion_medicationrecord where TransactionID='" + ViewState["TID"].ToString() + "' and IndentNo='" + IndentNo + "' and ItemID='" + ItemID + "'"));
                else
                    IssuedQty = Util.GetDecimal(StockReports.ExecuteScalar("Select Sum(ifnull(Qty,0)) from infusion_medicationrecord where TransactionID='" + ViewState["TID"].ToString() + "' and ItemID='" + ItemID + "'"));
                ...
                if (ItemID.Substring(0, 3).ToString().ToUpper() != "OTH")
                    ...
                    ReceivedQty = Util.GetDecimal(StockReports.ExecuteScalar("Select Sum(ifnull(ReqQty,0)) from orderset_infusionmedication where TransactionID='" + ViewState["TID"].ToString() + "' and MedicineID='" + ItemID + "'"));
                ...
                if (Session["RoleID"].ToString() != "52")
                {
                 ...
                 /*   if (txtQty.Text.Trim() == "")
                    ...
                    //    return;
                    //}
                    if (ddlSyringeContraction.SelectedIndex == 0)
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "key", "alert('Please Select Syringe Contraction');", true);
                        ddFreequency.Focus();
                        return;
                    }
                }
                ...
                string str = "";
                int syringechanged = 0;
                if (chkSyringeChanged.Checked)
                {
                    syringechanged = 1;
                }
                if (rdoStop.Checked == true)
                {
                    ...
                    str = "Insert into infusion_medicationrecord(TransactionID,IndentNo,ItemID,ItemName,IssueDate,DATE,TIME,Dose,EntryBy,StatusType,Remark,Qty,status,Frequency,Route,IVRate,SyringeContraction,SyringeContraction1,SyringeChanged,DripRate)values(";
                    str = str + " '" + ViewState["TID"].ToString() + "','" + IndentNo + "','" + ItemID + "','" + ItemName + "','" + IssueDate + "','" + MedicineDate + "',";
                    str = str + " '" + Util.GetDateTime(txttime.Text).ToString("HH:mm:ss") + "','" + txtDose.Text + "','" + ViewState["UserID"].ToString() + "','" + StatusType + "','" + Status + ": " + txtRemark.Text + "'," + Util.GetDecimal(txtQty.Text) + "," + status + ",'" + ddFreequency.SelectedItem.Text + "','" + ddlRoute.SelectedItem.Text + "','"+txtIVRate.Text+"','"+txtSyringeContraction.Text+"','"+ddlSyringeContraction.SelectedValue+"','"+syringechanged+"','"+txtDripRate.Text+"')";
                }
                else
                {
                    str = "Insert into infusion_medicationrecord(TransactionID,IndentNo,ItemID,ItemName,IssueDate,DATE,TIME,Dose,EntryBy,StatusType,Remark,Qty,Frequency,Route,IVRate,SyringeContraction,SyringeContraction1,SyringeChanged,DripRate)values(";
                    str = str + " '" + ViewState["TID"].ToString() + "','" + IndentNo + "','" + ItemID + "','" + ItemName + "','" + IssueDate + "','" + MedicineDate + "',";
                    str = str + " '" + Util.GetDateTime(txttime.Text).ToString("HH:mm:ss") + "','" + txtDose.Text + "','" + ViewState["UserID"].ToString() + "','" + StatusType + "','" + txtRemark.Text + "'," + Util.GetDecimal(txtQty.Text) + ",'" + ddFreequency.SelectedItem.Text + "','" + ddlRoute.SelectedItem.Text + "','" + txtIVRate.Text + "','" + txtSyringeContraction.Text + "','" + ddlSyringeContraction.SelectedValue + "','" + syringechanged + "','" + txtDripRate.Text + "')";
                }
    ...
    protected void rptMedicineDetails_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            ...
            ddlRouteHos.SelectedValue = "Intravenours (IV)";
    ...
    protected void btnSaveDose_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                bool IsSave = StockReports.ExecuteDML("UPDATE orderset_infusionmedication SET Dose='" + txtPopDose.Text + "',Timing='" + ddlPopTimes.SelectedItem.Text + "',Duration='" + Util.GetDateTime(txtPopDuration.Text).ToString("yyyy-MM-dd") + "',UpdatedBy='" + ViewState["UserID"].ToString() + "',UpdateDateTime=NOW() WHERE TransactionID='" + ViewState["TID"].ToString() + "' AND IndentNo='" + lblIndentNo.Text + "' AND MedicineID='" + lblPopItemID.Text + "'");
                if (IsSave)
                {
    ...
    protected void btnOtherMedicine_Click(object sender, EventArgs e)
    {
        ...
        var EntryID = StockReports.ExecuteScalar("Select Max(EntryID)+1 from orderset_infusionmedication");
        if (EntryID == "")
            EntryID = Util.GetInt("1").ToString();

        StockReports.ExecuteDML("INSERT INTO orderset_infusionmedication(EntryID,TransactionID,PatientID,MedicineID,MedicineName,ReqQty,Dose,Timing,Duration,Route,EntryBy,EntryDate,IsInHouseMedicine)VALUES('" + EntryID + "','" + ViewState["TID"].ToString() + "','" + ViewState["PID"] + "','OTH" + EntryID + "','" + txtOtherMedicine.Text + "','" + txtQtyIssued.Text + "','" + txtDose.Text + "','" + txtTime.Text + "','" + duration + "','" + ddlRoute.SelectedValue + "','" + ViewState["UserID"] + "',NOW()," + chkInHouse.Checked + ")");
