...
public partial class Design_Equipment_Masters_AssetRoomMaster : System.Web.UI.Page
{
    ...
    private void BindLocation()
    {
        DataTable dt = new DataTable();
            dt = StockReports.GetDataTable(" SELECT LocationName,LocationID from eq_location_master where IsActive=1 order by LocationName ");
            if (dt.Rows.Count > 0)
            {
    ...
    private void LoadData()
    {
        ...
        string str = "SELECT rm.`Department`,rm.DepartmentID, rm.LocationID, rm.RoomID,rm.RoomName,rm.RoomCode,rm.Description,";
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        if (CheckValidation())
        {
            ...
            try
            {
                ...
                {
                    str = "Insert into eq_Room_master(RoomName,RoomCode,LocationID,FloorID,LastUpdatedby,UpdateDate,IPAddress,IsActive,DepartmentID,Department,Description)";
                    str += "value('" + txtname.Text.Trim().Replace("'", "") + "','" + txtCode.Text.Trim() + "','" + ddlLocation.SelectedValue + "','" + ddlFloor.SelectedValue + "',";
                    str += "'" + ViewState["LastUpdatedby"].ToString() + "',Now(),'" + ViewState["IPAddress"].ToString() + "'," + IsActive + ",'" + Util.GetString(ddlDepartment.SelectedValue) + "','" + Util.GetString(ddlDepartment.SelectedItem.Text) + "','" + txtDescription.Text.Trim() + "')";
                }
                else
                {
                    //Getting Last Record
                    ...
                    "Department:',rm.Department,',', 'Description :', rm.Description)DataLog " +
                    "FROM eq_Room_master rm WHERE rm.RoomID=" + ViewState["RoomID"].ToString() + " ");


                    str = "Update eq_Room_master Set DepartmentID='" + Util.GetString(ddlDepartment.SelectedValue) + "',Department='" + Util.GetString(ddlDepartment.SelectedItem.Text) + "', RoomName='" + txtname.Text.Trim() + "',RoomCode='" + txtCode.Text.Trim() + "',IsActive = " + IsActive + ",";
    ...
    {
        if (Util.GetString(ddlDepartment.SelectedValue) == "0")
        {
    ...
    protected void AddFloor_Click(object sender, EventArgs e)
    {
        Response.Redirect("Floormaster.aspx");
