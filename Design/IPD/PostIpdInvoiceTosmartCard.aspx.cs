...
public partial class Design_IPD_PostIpdInvoiceTosmartCard : System.Web.UI.Page
{
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(" SELECT t.Type,t.CentreName,t.Patient_ID, PanelGroup,t.Pname,t.DispatchDate, t.panelAmount as BillAmount,t.billno,t.PanelInvoiceNo,t.PolicyNo,t.IsClaimPosted FROM  ");
        sb.Append(" ( ");
        sb.Append(" SELECT cm.`CentreName`,dis.Type,dis.PatientID AS Patient_ID,dis.panelid AS panel_id,pam.`PanelGroup`, ");
        ...
        sb.Append("    IFNULL(( SELECT IF(IFNULL(pds.IsClaimPosted,0)=1,1,0) FROM panel_amountallocation pds  ");
	     sb.Append("  WHERE pds.PanelID=dis.`PanelID` AND dis.`TransactionID`=pds.TransactionID ");
	    sb.Append("  ),'0')IsClaimPosted   ");
        sb.Append(" FROM f_dispatch dis ");
        ...
        sb.Append(" AND dis.`EntryDate`>=CONCAT('" + Util.GetDateTime(ucFromDate.Text).ToString("yyyy-MM-dd") + "',' 00:00:00') AND dis.`EntryDate`<=CONCAT('" + Util.GetDateTime(ucToDate.Text).ToString("yyyy-MM-dd") + "',' 23:59:59') AND pmh.`TYPE`='IPD' ");
        sb.Append(" )t ");
