<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PatientPanelApproval.aspx.cs" Inherits="Design_IPD_PatientPanelApproval" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#txtPID').val('<%= Request.QueryString["PatientId"].ToString()%>');
            var PatientID = '<%= Request.QueryString["PatientId"].ToString()%>';
            if (PatientID != "") {
                serverCall('../Common/CommonService.asmx/BindPatientMultiPanelDetails', { PatientID: PatientID }, function (response) {
                    var responseData = JSON.parse(response);
                    if (responseData.length > 0)
                        $getMultiPanelDetails(responseData, function () { });
                });
            }
            $('#txtPanelApprovalAmount').addClass('requiredField');
        });
        ...
        var sendApprovalEmailToPanel = function (callback) {
            ...
            if ($('#txtPanelApprovalAmount').val() == "") {
                modelAlert('Please Enter Approval Amount');
                return;
            }

            if ($("#lblIsSmartCardIpd").text() == "1") { 
                
                if ($("#lblisDataFetchIpd").text() == "1") {
                    ...
                    if (PoolNr == 0 || PoolNr == "0" || PoolNr == "" || PoolNr == undefined || PoolNr == null) {
                        ...
                        SavePatientPanelDetails(1, 0, PoolNr, PoolDesc)
                    }
        ...
        function SavePatientPanelDetails(IsSamrtCard,IsManual,PoolNo,PoolName) {
            ...
            getApprovalEmailToPanel(function (data) {
                ...
                data.IsThroughsmartCard=IsSamrtCard;               
                data.IsThroughManul = IsManual;
                data.PoolNr = PoolNo;
                data.PoolDesc = PoolName;
