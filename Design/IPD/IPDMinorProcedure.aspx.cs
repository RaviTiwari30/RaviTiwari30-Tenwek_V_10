...
public partial class Design_IPD_IPDMinorProcedure : System.Web.UI.Page
{
    ...
    public DataTable BindItem()
    {
        ...
        string searchType = Util.GetString(Request.QueryString["type"]);
        ...
        sb.Append(") RL ON RL.ItemID=IM.ItemID WHERE cf.ConfigID=25 AND im.Isactive=1 AND itc.`IsActive`=1 AND itc.CentreID='" + Session["CentreID"].ToString() + "' ");
       
        if(searchType =="1")
        sb.Append(" AND TypeName like '%" + itemName.Trim() + "%' ");
        else
            sb.Append(" AND RL.ItemCode='" + itemName.Trim() + "' ");
