...
public partial class Design_MIS_Analysis : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            txtFromDate.Text = DateTime.Now.Date.AddDays(-31).ToString("dd-MMM-yyyy");
    ...
    [WebMethod]
    public static string ProfitSummary(string fromDate, string toDate, string CentreID)
    {
        string result = "0";
    ...
    [WebMethod]
    public static string InventoryAnalysis(string fromDate, string toDate,string CentreID)
    {
        string result = "0";
        ...
        query.Append(" UNION ALL SELECT 'Stock Available for Next' AS Title,CONCAT(FLOOR(a.StockValue/b.AvgDailySale),'') AS Amount,'0' AS IsRupee ");
    ...
    [WebMethod]
    public static string TopSellingProducts(string fromDate, string toDate, string CentreID)
    {
        string result = "0";

        StringBuilder query = new StringBuilder();
        query.Append(" SELECT t1.ItemName,ROUND((t1.Amt),2)Amount,ItemID FROM (SELECT s.ItemName,SUM(sd.PerUnitSellingPrice*sd.SoldUnits)Amt,sd.ItemID FROM f_salesdetails sd  INNER JOIN f_stock s ON s.StockID=sd.StockID ");
        ...
        query.Append(" UNION ALL SELECT 'Others' ItemName,ROUND(SUM(Amt),2)Amount,ItemID FROM (SELECT s.ItemName,SUM(sd.PerUnitSellingPrice*sd.SoldUnits)Amt,sd.ItemID FROM f_salesdetails sd  INNER JOIN f_stock s ON s.StockID=sd.StockID ");
    ...
    [WebMethod]
    public static string TopPurchasedProducts(string fromDate, string toDate, string CentreID)
    {
        string result = "0";

        StringBuilder query = new StringBuilder();
        query.Append(" SELECT * FROM (SELECT ltd.ItemName,ROUND(SUM(ltd.Amount),0)Amount, ltd.ItemID FROM f_ledgertransaction lt INNER JOIN f_ledgertnxdetail ltd ON ltd.LedgerTransactionNo = lt.LedgerTransactionNo ");
        ...
        query.Append(" UNION ALL SELECT 'Others' ItemName,ROUND(SUM(Amount),0)Amount,ItemID FROM (SELECT ltd.ItemName,SUM(ltd.Amount)Amount,ltd.ItemID FROM f_ledgertransaction lt INNER JOIN f_ledgertnxdetail ltd ON ltd.LedgerTransactionNo = lt.LedgerTransactionNo ");
    ...
    [WebMethod]
    public static string ConsultantStatistics(string fromDate, string toDate, string CentreID)
    {
        string result = "0";

        StringBuilder query = new StringBuilder();
        query.Append(" SELECT dm.Name Doctor,SUM(lt.NetAmount)Amount,COUNT(pmh.TransactionID)NoOfPrescriptions,pmh.DoctorID FROM f_ledgertransaction lt INNER JOIN patient_medical_history pmh ON pmh.TransactionID = lt.TransactionID ");
    ...
    [WebMethod]
    public static string TopSalesperson(string fromDate, string toDate, string CentreID)
    {
        string result = "0";

        StringBuilder query = new StringBuilder();
        query.Append(" SELECT Employee,NoOfOrders,Amount,ROUND((Amount/TotalSale)*100,2)Percent,EmployeeID FROM ( SELECT CONCAT(em.Title,' ',em.Name)Employee,SUM(lt.NetAmount)Amount,COUNT(lt.LedgerTransactionNo)NoOfOrders,em.EmployeeID FROM f_ledgertransaction lt INNER JOIN employee_master em ON em.EmployeeID = lt.UserID ");
    ...
    [WebMethod]
    public static string TopSuppliers(string fromDate, string toDate, string CentreID)
    {
        string result = "0";

        StringBuilder query = new StringBuilder();
        query.Append(" SELECT LedgerName,NetPurchase,No_Of_GRN,ROUND((NetPurchase/TotalSale)*100,2)Percent,LedgerNumber FROM (SELECT lm.LedgerName, ROUND(SUM(lt.NetAmount+lt.RoundOff))NetPurchase,COUNT(lt.LedgerTransactionNo)No_Of_GRN ,lm.LedgerNumber FROM f_ledgertransaction lt INNER JOIN f_ledgermaster lm ON lm.LedgerNumber = lt.LedgerNoCr ");
