<%@ Page Title="" Language="C#" MasterPageFile="~/DefaultHome.master" AutoEventWireup="true" CodeFile="CreatePurchaseRequest.aspx.cs" Inherits="Design_Store_CreatePurchaseRequest" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        var getCategorys = function (storeID,callback) {
            serverCall('Services/CommonService.asmx/GetCategorysByStoreType', { storeID: storeID }, function (response) {
                var responseData = JSON.parse(response);
                callback($('#ddlCategory').bindDropDown({ data: JSON.parse(response), valueField: 'CategoryID', textField: 'Name' }));
	        	GetSubCatgryOnLoad();
            });
        ...
        var getCentre = function (callback) {
            serverCall('Services/CommonService.asmx/GetALLCentre', {}, function (response) {
                ...
                _ddlCentre.bindDropDown({ defaultValue: 'Select', data: JSON.parse(response), valueField: 'CentreID', textField: 'CentreName', selectedValue: centreID });
        ...
        var getDepartMent = function (callback) {
            serverCall('Services/CommonService.asmx/GetDepartMent', {}, function (response) {
                var responseData = JSON.parse(response);
                callback($('#ddlDepartmentTo').bindDropDown({ defaultValue: 'Select', data: JSON.parse(response), valueField: 'DeptLedgerNo', textField: 'RoleName' }));
            });
        }
        ...
        var configureTable = function (s) {
            ...
            hTables.addHook('beforeChange', function (changes, source) {
                ...
                var h = this;
                var quantityTypes = ['Quantity'];//['Quantity', 'IndentQuantity', 'SalesQuantity'];
                if (prop === 'ItemName') {
                    ...
                    getItemStockDetails(selectedData.ItemID, function (itemDetails) {
                        h.setDataAtCell(row, h.propToCol('Minlevel'), itemDetails[0].MinLevel);
                        h.setDataAtCell(row, h.propToCol('Maxlevel'), itemDetails[0].MaxLevel);
                        h.setDataAtCell(row, h.propToCol('CurrentStock'), itemDetails[0].StockQuantity);
                        h.setDataAtCell(row, h.propToCol('PoStock'), itemDetails[0].POQuantity);
                        h.setDataAtCell(row, h.propToCol('Quantity'), 0);
                        h.setDataAtRowProp(row, h.propToCol('SubCategoryID'), selectedData.SubCategoryID);
                        h.setDataAtCell(row, h.propToCol('PurchaseUnit'), selectedData.MajorUnit);
			            h.setDataAtCell(row, h.propToCol('ConversionFactor'), selectedData.ConversionFactor);
                        h.setDataAtCell(row, h.propToCol('Narration'), '');
                        h.setDataAtRowProp(row, h.propToCol('ItemID'), selectedData.ItemID);
                        h.setDataAtRowProp(row, h.propToCol('IndentNumber'), '0');
                        h.setDataAtRowProp(row, h.propToCol('IndentQuantity'), '0');
                        h.setDataAtRowProp(row, h.propToCol('AvgConsumption'), '0');
                        h.setDataAtRowProp(row, h.propToCol('DepartmentStock'), '0');
                        h.setDataAtRowProp(row, h.propToCol('MinConsumption'), '0');
                        h.setDataAtRowProp(row, h.propToCol('SalesQuantity'), '0');
                        h.setDataAtCell(row, h.propToCol('ReorderLevel'), itemDetails[0].ReorderLevel);
                        h.setDataAtCell(row, h.propToCol('ReorderQty'), itemDetails[0].ReorderQty);
        ...
        var getGridSetting = function (callback) {
            ...
            var s = {
                ...
                colWidths: [320, 55, 76, 85, 85, 63, 68, 50, 100, 70, 80, 150],
                //colHeaders: ['ItemName', 'Qty.', 'Sales Qty.', 'Indent Qty.', 'Net Qty.', 'Minlevel', 'Maxlevel', 'Stock', 'Pending PO Qty.', 'Unit', 'C-Factor', 'Remarks'],
                colHeaders: ['ItemName', 'Qty.', 'Cur.Stock', 'ReorderLevel', 'ReorderQty', 'Net Qty.', 'Minlevel', 'Maxlevel', 'Pending PO Qty.', 'Unit', 'C-Factor', 'Remarks'],
                columns: [
					{
					    type: 'autocomplete',
					    data: 'ItemName',
                    ...
                    //{ data: 'IndentQuantity', type: 'numeric', readOnly: true },
                    ...
                    //{ data: 'PoStock', type: 'numeric', readOnly: true },
                    //{ data: 'PurchaseUnit', type: 'text', readOnly: true },
                    //{ data: 'ConversionFactor', type: 'text', readOnly: true },
					//{ data: 'Narration', type: 'text' },

                    { data: 'Quantity', type: 'numeric', format: quantityFormat },
                    	{ data: 'CurrentStock', type: 'numeric', readOnly: true },
                    { data: 'ReorderLevel', type: 'numeric', readOnly: true },
                    { data: 'ReorderQty', type: 'numeric', readOnly: true },
                    { data: 'NetQuantity', type: 'numeric', readOnly: true },
					{ data: 'Minlevel', type: 'numeric', readOnly: true },
					{ data: 'Maxlevel', type: 'numeric', readOnly: true },
        ...
        var getItems = function (callback, isStoreTypeChange) {
            if (items.length == 0 || isStoreTypeChange == 1) {
                ...
                serverCall('Services/CreatePurchaseRequest.asmx/GetItems', data, function (response) {
                    var responseData = JSON.parse(response);
                    items = responseData;
                    callback(items);
                });
        ...
        var getItemStockDetails = function (itemID, callback) {
            ...
            serverCall('Services/CreatePurchaseRequest.asmx/getItemStockDetails', data, function (response) {
                var responseData = JSON.parse(response);
                callback(responseData);
            });
        }
        ...
        var getAutoPurchaseRequestItemsOnSales = function () {
            ...
            serverCall('Services/CreatePurchaseRequest.asmx/GetAutoPurchaseRequestItems', data, function (response) {
                var responseData = JSON.parse(response);
                getGridSetting(function (s) {
                    s.data = responseData;
                    configureTable(s);
                ...
                serverCall('Services/CreatePurchaseRequest.asmx/Save', data, function (response) {
                    ...
                    modelAlert(responseData.message, function () {
                        if (responseData.status) {
                            ...
                            if (!String.isNullOrEmpty(editPurchaseRequestNo))
                                location.href = 'CreatePurchaseRequest.aspx';
                            else
                                location.reload();
                        }
                        else {
    ...
    <div id="Pbody_box_inventory">
        ...
        <div class="POuter_Box_Inventory" style="text-align: center;">
            <b id="pageTitle">Create Purchase Request</b>
        </div>
        <div class="POuter_Box_Inventory">
            <div class="Purchaseheader">
                Auto Purchase Request Behalf Of Sales  
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-5">
                    <select id="ddlCenterTo" class="required"></select>
                </div>
		...
		<div class="col-md-3">
            ...
            <div class="row" style="text-align:center">
                <div class="col-md-24">
                    ...
                    <input type="button" style="margin-right: 5px;" onclick="getAutoPurchaseRequestItemsOnSales()" value="Auto Purchase Request On ReOrder Level" />
                </div>
            </div>
        </div>
        <div class="POuter_Box_Inventory">
            ...
            <div class="row">
                ...
                <div class="col-md-5">
                    <select id="ddlRequestionType">
                        ...
                        <option value="4" style="display:none">Amended</option>
                    </select>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-5">
                    <asp:TextBox runat="server" ClientIDMode="Static" ID="txtRequisitionOn"></asp:TextBox>
        ...
        <div class="POuter_Box_Inventory textCenter">
            ...
            <input type="button" id="btnCancel" value="Cancel" class="save margin-top-on-btn" style="display:none"  onclick="location.href = 'CreatePurchaseRequest.aspx'" />
        </div>
