...
public partial class Design_Emergency_Alerts : System.Web.UI.Page
{
    protected void BindData(string TID)
    {
        try
        {
            string alert = StockReports.ExecuteScalar("SELECT Alert FROM emergency_patient_details WHERE TransactionID='" + TID + "'");
            if (alert != "")
            {
                txtAlert.Text = alert.Replace("@", "'");
            }
            else
            {
                txtAlert.Text = "";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (ChkEntery > 0)
            {
                MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "UPDATE Emergency_Patient_Details SET Alert='" + txtAlert.Text.Replace("'", "@").Trim() + "' WHERE TransactionID='" + ViewState["TID"].ToString() + "'");
            }
            else
            {
