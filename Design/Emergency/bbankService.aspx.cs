...
public partial class Design_IPD_bbankService : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ViewState["EmergencyNo"]=Request.QueryString["EMGNo"].ToString();
            ...
            BindBloodGroup();
            RefferedByBind();
            getEmergencyPatientDetails(ViewState["EmergencyNo"].ToString());
    ...
    public void RefferedByBind()
    {
        ...
        cmbRefferedBy.Items.Insert(0, new ListItem("Select","0"));
    }
    ...
    protected void btnReceipt_Click(object sender, EventArgs e)
    {
        if (dtItem != null && dtItem.Rows.Count > 0)
        {
            ...
            try
            {
                ...
                string IsVerified = "1";
                dtItem = IPDBilling.UpdateLedgerForIPDBilling(dtItem, PID, "Emergency", Util.GetInt(PanelID), ViewState["ID"].ToString(), TID, Util.GetString(Session["HOSPID"].ToString()), "Yes", IsVerified, tnx, IPDCaseType_ID, lblPatientType.Text.Trim(), con, lblRoomID.Text.Trim());
                ...
                for (int a = 0; a < dtItem.Rows.Count; a++)
                {
                    ...
                    sb.Append(" DoctorID,PanelID,TypeID,TYPE,EntryBy,ConfigID,PanelCurrencyCountryID,PanelCurrencyFactor,CentreID,TransactionID,PatientID,RoleID,EmergencyNo,Patient_Type) ");
                    ...
                    sb.Append(" '" + dtItem.Rows[a]["DoctorID"].ToString() + "', " + Util.GetInt(PanelID) + ", " + dtItem.Rows[a]["TnxTypeID"].ToString() + ",'BB','" + ViewState["ID"].ToString() + "'," + Util.GetInt(dtItem.Rows[a]["ConfigID"]) + ",");
                    sb.Append(" 0,0," + Util.GetInt(ViewState["CentreID"].ToString()) + ",'" + TID + "','" + PID + "'," + Util.GetInt(Session["RoleID"]) + ",'" + ViewState["EmergencyNo"].ToString() + "','EMG') ");
        ...
        try
        {
            ...
            if (IsExist == string.Empty)
            {
               // DataTable dtTemp = IPDBilling.LoadItemsHavingRateFixed("'" + ItemID + "'", Ref_Code, IPDCaseType_ID, ViewState["ScheduleChargeID"].ToString(), ViewState["CentreID"].ToString());
                ...
                string RefID = StockReports.ExecuteScalar(" SELECT ReferenceCodeOPD FROM f_panel_master WHERE PanelID =" + lblPanelID.Text + " ");
                DataTable dtTemp = StockReports.GetDataTable("select (IFNULL(rt.Rate,0))Rate,rt.ScheduleChargeID,rt.ID,IFNULL(rt.ItemCode,'')ItemCode,IFNULL(rt.ItemDisplayName,'')ItemDisplayName  from f_ratelist rt INNER JOIN f_rate_schedulecharges rsc ON rsc.PanelID = rt.PanelID AND rsc.IsDefault=1 AND rsc.ScheduleChargeID=rt.ScheduleChargeID where rt.ItemID='" + ItemID + "' and rt.PanelID=" + RefID + " and IsCurrent=1 and rt.CentreID='" + Session["CentreID"].ToString() + "'");
    ...
    private void getEmergencyPatientDetails(string EmergencyNo)
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("  SELECT pm.`PatientID`,epd.`EmergencyNo`,CONCAT(pm.`Title`,' ',pm.`PName`)'Name',CONCAT(pm.`Age`,'/',pm.`Gender`)'AgeSex',pm.bloodgroup,PMH.Patient_Type PatientType,   ");
            sb.Append("  CONCAT(dm.`Title`,' ',dm.`Name`)'Doctor',pnl.`Company_Name` 'Panel',DATE_FORMAT(epd.`EnteredOn`,'%d-%b-%Y %h:%i %p')'InDateTime',PMH.ScheduleChargeID,    ");
            ...
            sb.Append("  ,lt.`LedgerTransactionNo` LTnxNo,CONCAT(rm.`Name`,'/',rm.`Room_No`,'/',rm.`Bed_No`)'Room',rm.`RoomId`,pnl.`PanelID`,pnl.ReferenceCode,epd.`IPDCaseTypeID`,dm.`DoctorID` ");
            sb.Append(" ,lt.`GrossAmount`,lt.`DiscountOnTotal`,lt.`NetAmount`,lt.`RoundOff`,lt.`BillNo`,IF(lt.`BillNo`<>'',DATE_FORMAT(lt.`BillDate`,'%d-%b-%Y'),'')'BillDate'  ");
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTransactionNo.Text = dt.Rows[0]["TID"].ToString();
    ...
    private void BindPatientDetails(string TransactionID)
    {
        try
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                lblCaseTypeID.Text = dt.Rows[0]["IPDCaseType_ID"].ToString();
                lblReferenceCode.Text = dt.Rows[0]["ReferenceCode"].ToString();
                lblPanelID.Text = dt.Rows[0]["PanelID"].ToString();
                lblDoctorID.Text = dt.Rows[0]["DoctorID"].ToString();
