...
public partial class PreOPRecord : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            int ChkEntery = Util.GetInt(MySqlHelper.ExecuteScalar(tnx, CommandType.Text, "SELECT COUNT(*) FROM preoprecord WHERE ID='" + lblID.Text + "'"));
            if (ChkEntery > 0)
            {
                MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "delete from preoprecord WHERE ID='" + lblID.Text + "'");
            }
            string chklistValue = "";
            ...
            sb.Append(" INSERT INTO preoprecord(Patient_ID,Transaction_ID,Appointment_ID,OTBOOKING_ID,OriginalWard,ArrivalRecovery,DepartureRecovery, ");
            sb.Append(" Surgeon, Anaesthist, CONDITIONN, Conscious, RP, Colour, SemiConscious, PulseRate, Conjuntiva, Unconscious,");
            sb.Append("  SPO2, Respiration, Adequate, Depressed, Vomited, Salivation, Pharynx, edication, IVFluids, ");
            sb.Append("  Remarks,CreatedBy,CreatedDate,Temp,BloodSugar,BloodSugarType,OtherSpecify,AdditionalDoctors)VALUES ('" + PatientId.Text + "','" + Transaction_ID.Text + "','" + App_ID.Text + "', '" + OTBookingID.Text + "', '" + txtOriginalWard.Text.Trim().ToString() + "', '" + Util.GetDateTime(txtArrivalRecovery.Text).ToString("HH:mm:ss") + "', '" + Util.GetDateTime(txtDepartureRecovery.Text).ToString("HH:mm:ss") + "', ");
            ...
            sb.Append(" '" + txtRespiration.Text.Trim().ToString() + "','" + txtAdequate.Text.Trim().ToString() + "','" + txtDepressed.Text.Trim().ToString() + "','" + rdList.SelectedValue + "', ");
            sb.Append("'" + chklistValue.TrimStart(',') + "','" + chklistValue2.TrimStart(',') + "','" + txtMedication.Text.Trim().ToString() + "','" + txtIVFluids.Text.Trim().ToString() + "','" + txtRemarks.Text.Trim().ToString() + "','" + ViewState["UserID"] + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + txtTemp.Text.Trim() + "','" + ddlParticular.SelectedValue.ToString() + "','" + txtbloodsugar.Text + "','" + txtOtherSpecification.Text + "','" + txtAdditionalDoctors.Text + "')");
    ...
    protected void grdPhysical_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "Change")
            {
                ...
                DataTable dtdetail = StockReports.GetDataTable(" select *,DATE_FORMAT(ArrivalRecovery,'%h:%i %p')ArrivalRecovery1,DATE_FORMAT(DepartureRecovery,'%h:%i %p')DepartureRecovery1 from preoprecord WHERE ID='" + id + "'");
            ...
            {
                string id = e.CommandArgument.ToString();
                string str1 = "SELECT DATE_FORMAT(pr.ArrivalRecovery,'%h:%i %p')ArrivalRecovery1,DATE_FORMAT(pr.DepartureRecovery,'%h:%i %p')DepartureRecovery1,pr.OriginalWard,pr.ReturnedWard,UPPER(CONCAT(dm.Title,' ',dm.Name))SurgeonName,UPPER(CONCAT(dj.Title,' ',dj.Name))AnaesthistName, pr.CONDITIONN,pr.Conscious, pr.RP, pr.Colour, pr.SemiConscious, pr.PulseRate, pr.Conjuntiva, pr.Unconscious, pr.SPO2, pr.TimeFullyConscious, pr.Respiration, pr.Adequate, pr.Depressed,pr.Vomited,pr.Salivation,pr.Pharynx,pr.edication,pr.IVFluids,pr.Remarks FROM preoprecord pr LEFT JOIN  doctor_master dm ON dm.DoctorID=pr.Surgeon LEFT JOIN  doctor_master dj ON dj.DoctorID=pr.Anaesthist WHERE pr.ID='" + id + "'";
    ...
    private void BindGraphBP(string TID)
    {
        try
        {
            ...
            {
                dt = StockReports.GetDataTable("SELECT split_str(Rp,'/',1)Systolic,TRIM(split_str(Rp,'/',2))Diastolic,date_format(CreatedDate,'%d-%b-%Y %H:%i %p') as CreatedDate FROM preoprecord WHERE Transaction_ID='" + TID + "'");
            }
    ...
    private void BindGraphPulse(string TID)
    {
        try
        {
            ...
            if (TID.Contains("ISHHI"))
            {
                dt = StockReports.GetDataTable("SELECT (SPO2+0)SPO2,date_format(CreatedDate,'%d-%b-%Y %H:%i %p') as CreatedDate FROM preoprecord WHERE Transaction_ID='" + TID + "' ");
            }
