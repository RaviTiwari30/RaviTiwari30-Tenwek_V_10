...
public partial class Design_OT_PreOPvitalEntry : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string str = "insert into Ot_preop_vitals (Date,Time,Temp,Pulse,Resp,Bp,Wound,Drains,Comments,TransactionID,Patient_ID,CreatedBy,CreatedDate,Isactive,BloodSugar,Weight,Oxygen,POD,Height,BMI,BSA) Values ('" + Util.GetDateTime(txtDate.Text).ToString("yyyy-MM-dd") + "','" + Util.GetDateTime(txtTime.Text).ToString("HH:mm") + "','" + TempConvert + "','" + txtPulse.Text.Trim() + "','" + txtResp.Text.Trim() + "','" + txtBP.Text.Trim() + "','" + txtwounds.Text.Trim() + "','" + txtDrains.Text.Trim() + "','" + txtComment.Text.Trim() + "','" + Util.GetString(ViewState["TransactionID"]).Trim() + "','" + Util.GetString(ViewState["PatientID"]).Trim() + "','" + ViewState["UserID"] + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','1','" + txtBloodSugar.Text.Trim() + "','" + txtWeight.Text.Trim() + "','" + txtOxygen.Text.Trim() + "','" + txtPOD.Text.Trim() + "','" + txtHeight.Text.Trim() + "','" + txtBMI.Text.Trim() + "','" + txtBSA.Text.Trim() + "')";
    ...
    private DataTable Search()
    {
        ...
        sb.Append(" SELECT ipo.Height,ipo.BMI,ipo.BSA,ipo.ID,DATE_FORMAT(ipo.DATE,'%d-%b-%Y') AS DATE,Date_Format(ipo.Time, '%l:%i %p') Time,FORMAT(ipo.Temp,1)Temp,ipo.POD, ");
        sb.Append(" FORMAT(ipo.pulse,0)pulse,FORMAT(ipo.resp,0)resp,ipo.BP,ipo.wound,ipo.drains,ipo.BloodSugar,ipo.Weight,ipo.Oxygen,ipo.comments, ");
        sb.Append(" Replace(ipo.TransactionID,'ISHHI','')IPDNo,ipo.Patient_ID,ipo.CreatedBy, ");
        sb.Append(" (Select concat(title,' ',name) from Employee_master where EmployeeID=ipo.CreatedBy)EmpName,CONCAT(dm.title,'',dm.name) AS DoctorName, ");
        sb.Append(" CONCAT(pm.Title,'',pm.PName) AS PatientName,pmh.DateOfVisit,CONCAT(em.title,'',em.name) AS Username,TIMESTAMPDIFF(MINUTE,ipo.CreatedDate,NOW())createdDateDiff  ");
        sb.Append(" FROM Ot_preop_vitals ipo ");
    ...
    public static string Bindgraph_Pulse(string TID)
    {
        ...
        sb.Append(" FROM Ot_preop_vitals  ");
    ...
    public static string Bindgraph_BP(string TID)
    {
        ...
        sb.Append(" FROM Ot_preop_vitals WHERE TransactionID='" + TID + "' ");
