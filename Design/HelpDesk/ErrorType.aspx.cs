...
public partial class HelpDesk_Error_Type : System.Web.UI.Page
{
    string CurentID = "";
    ...
    {
        DataTable dt = StockReports.GetDataTable(" select ID,RoleName from f_rolemaster where active=1 order by RoleName ");// and isAssetServiceProvider=1
        if (dt.Rows.Count > 0)
        {
    ...
    public void BindEmployee() // developed by Ankit
    {
        ...
        string query = "SELECT EmployeeID, NAME FROM Employee_master WHERE IsActive=1 AND NAME<>'' order by Name";// and isAssetServiceProvider=1
