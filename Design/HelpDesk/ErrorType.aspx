<%@ Page Language="C#" MasterPageFile="~/DefaultHome.master" AutoEventWireup="true" CodeFile="ErrorType.aspx.cs" Inherits="HelpDesk_Error_Type"  %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script runat="server">
     ...
     <script type="text/javascript">
        ...
        function errorTypeSearch() {
            $.ajax({
                type: "POST",
                url: "Services/HelpDesk.asmx/errorTypeSearch",
                data: '{}',
                dataType: "json",
        ...
        function saveErrorType() {
            ...
            if ($.trim($("#txtErrorType").val()) == "") {
                $("#btnSave").val("Save").removeAttr("disabled", "disabled");
                //$("#lblErrormsg").text('Please Enter Error Type');
                modelAlert("Please Enter Error Type");
                $("#txtErrorType").focus();                
                return;
            }
            //if ($.trim($("#ddlEmployee").val()) == "0") {
            //    $("#btnSave").val("Save").removeAttr("disabled", "disabled");
            //    //    $("#lblErrormsg").text('Please Enter Error Type');
            //    modelAlert("Please Select Handled By  Employee");
            //    $("#ddlEmployee").focus();
            //    return;
            //}

            var employeeId = "";
            $.each($('#<%=ddlEmployee.ClientID%> input[type=checkbox]:checked'), function () {
                employeeId += $(this).val() + ",";
            });
            employeeId = employeeId.slice(0, -1);

            var empIDD = employeeId;
            
            var empiddefault = empIDD.substr(0, empIDD.indexOf(','));
            //alert(empiddefault);

            var selectedvals = "";
            $.each($('#<%=ddlAditionalInfo.ClientID%> input[type=checkbox]:checked'), function () {
            ...
            $.ajax({
                type: "POST",
                url: "Services/HelpDesk.asmx/saveErrorType",
                data: '{ErrorType:"' + $.trim($("#txtErrorType").val()) + '",Department:"' + $("#ddlDepartment").val() + '",Status:"' + $("#<%=rbtnStatus.ClientID%> input[type:radio]:checked").val() + '",EmployeeID:"' + $('#ddlEmployee').val() + '", AdditionalID:"' + selectedvals + '",defaultEmpID:"' + empiddefault + '"}',
                dataType: "json",
                contentType: "application/json;charset=UTF-8",
                async: false,
                success: function (response) {
        ...
        function errorTypeCon() {
            if ($("#btnSave").val() == "Save") {
                ...
                $("#lblErrormsg").text('');
                updateErrorType(errorID);
            }
        }
        function updateErrorType(errorID) {
            if ($.trim($("#txtErrorType").val()) == "") {
                $("#lblErrormsg").text('Please Enter Error Type');
                $("#txtErrorType").focus();
                return;
            ...
            $.ajax({
                type: "POST",
                url: "Services/HelpDesk.asmx/updateErrorType",
                data: '{ErrorType:"' + $.trim($("#txtErrorType").val()) + '",Department:"' + $("#ddlDepartment").val() + '",Status:"' + $("#<%=rbtnStatus.ClientID%> input[type:radio]:checked").val() + '",errorID:"' + errorID + '",EmployeeID:"' + $('#ddlEmployee').val() + '",additionalId:"' + selectedvals + '"}',
                dataType: "json",
                contentType: "application/json;charset=UTF-8",
                async: false,
                success: function (response) {
        ...
        function editErrorType(rowid) {
            var errorType = $(rowid).closest('tr').find('#tdErrorType').text();
            errorID = $(rowid).closest('tr').find('#tdErrorID').text();
            ...
            var roleID = $(rowid).closest('tr').find('#tdRoleID').text();
            $("#ddlDepartment").prop('disabled', 'disabled');
            $("#txtErrorType").val(errorType);
            //$("#ddlDepartment").val(roleID);
            $('#ddlDepartment').chosen('destroy').val(roleID).chosen();
            var empID = $(rowid).closest('tr').find('#tdEmployeeID').text().trim();
           // $("#<%=ddlEmployee.ClientID%> input[type=checkbox]").prop("checked", false);
            //empID = empID.split(',');
         //   for (var i = 0; i < empID.length; i++) {
               // $("#<%=ddlEmployee.ClientID%> [value=" + empID[i] + "]").attr("checked", "checked");
           // }

            // $("#ddlEmployee").val($(rowid).closest('tr').find('#tdEmployeeID').text());
            $('#ddlEmployee').chosen('destroy').val(empID).chosen();
            $("#btnSave").val('Update');
            $("#btnCancel").show();


            DropDownCheck(errorID);
        }
    ...
    <div id="Pbody_box_inventory">
        ...
        <div class="POuter_Box_Inventory">
            <div class="Purchaseheader">Issue Type</div>
            <div class="row">
               <div class="col-md-1"></div>
                <div class="col-md-22">
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <asp:DropDownList ID="ddlDepartment" TabIndex="1" runat="server" ClientIDMode="Static"  ToolTip="Select Department" CssClass="requiredField"></asp:DropDownList>
                        </div>
                        <div class="col-md-3"><label class="pull-left">Issue Type</label>
                            <b class="pull-right">:</b>
                        </div>
                        <div class="col-md-5">
                            <input type="text" maxlength="50" class="requiredField" id="txtErrorType" style="width: 200px" tabindex="2" onkeypress="return check(this,event)" onkeyup="validatespace();" title="Enter Error Type"  />
                        </div>
                        <div class="col-md-3" style="display:none;"><label class="pull-left">Handled By</label>
                            <b class="pull-right">:</b>
                        </div>
                        <div class="col-md-5" style="display:none;">
                            <asp:DropDownList ID="ddlEmployee" runat="server" ClientIDMode="Static" ToolTip="Select Employee" CssClass="requiredField">
                            </asp:DropDownList>

                            <%--<asp:DropDownCheckBoxes ID="ddlEmployee" runat="server" AddJQueryReference="True" UseButtons="false" UseSelectAllNode="True" Height="23px" CssClass="requiredField requiredField1">
        ...
        <div class="POuter_Box_Inventory">
            <div class="Purchaseheader">Error Types</div>
    ...
    <script id="tb_SearchErrorType" type="text/html">
        <table class="FixedTables" cellspacing="0" rules="all" border="1" id="tb_grdErrorType" style="width:100%;border-collapse:collapse;">
            <tr id="Header">
                ...
                <th class="GridViewHeaderStyle" scope="col" style="width:250px;">Issue Type</th>
                <th class="GridViewHeaderStyle" scope="col" style="width:200px;">Department</th>
                <th class="GridViewHeaderStyle" scope="col" style="width:200px;display:none;">Handled By</th>
                <th class="GridViewHeaderStyle" scope="col" style="width:90px;">Status</th>	
                <th class="GridViewHeaderStyle" scope="col" style="width:60px; display:none ">RoleID</th>		          
                <th class="GridViewHeaderStyle" scope="col" style="width:60px; display:none ">ErrorID</th>			 
                <th class="GridViewHeaderStyle" scope="col" style="width:60px; display:none ">EmployeeID</th>			 
            ...
            <tr id="<#=j+1#>">                            
                    ...
                    <td class="GridViewLabItemStyle" id="tdErrorType"  style="width:250px;text-align:center" ><#=objRow.Error_type#></td>
                    <td class="GridViewLabItemStyle" id="tdRolename"  style="width:200px;text-align:center" ><#=objRow.RoleName#></td>
                    <td class="GridViewLabItemStyle" id="td1"  style="width:200px;text-align:center;display:none;">
                        <#=objRow.EmployeeName#>
                     <%--   <label id='cls_<#=j+1#>'><#=j+1#></label>--%>
                    </td>
                    <td class="GridViewLabItemStyle" id="tdIsActive"  style="width:90px;text-align:center" ><#=objRow.IsActive#></td>
                    <td class="GridViewLabItemStyle" id="tdRoleID" style="width:60px;display:none"><#=objRow.RoleID#></td>
                    <td class="GridViewLabItemStyle" id="tdErrorID" style="width:60px;display:none"><#=objRow.Error_ID#></td>
                <td class="GridViewLabItemStyle" id="tdEmployeeID" style="width:60px;display:none">
                    <#=objRow.EmployeeID#>
                </td>
