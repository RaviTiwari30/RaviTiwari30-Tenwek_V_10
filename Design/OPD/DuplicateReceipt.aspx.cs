...
using System.Linq;
public partial class Design_OPD_DuplicateReceipt : System.Web.UI.Page
{
    ...
    public DataTable GetReceiptDetailnew(string FromDate, string ToDate, string ReceiptNo, string BillNo, string PatientName, string PatientID)
    {
        ...
        try
        {
          ...
          //  sb.Append(" SELECT '" + IsAllowedOriginalPrint + "' IsAllowedOriginalPrint,");
            sb.Append(" SELECT rp.Type,if(rp.Type='OPD','true','false') as IsCombinedPrint,Concat(PM.PatientID,'#',rp.PanelID) As CombinedID, rp.PanelID,rp.PanelName,'" + IsAllowedOriginalPrint + "' IsAllowedOriginalPrint,");
            
            sb.Append("PM.PatientID, PM.PName,CONCAT(IFNULL(PM.House_No,''),' ',IFNULL(PM.Street_Name,''),' ',IFNULL(PM.Locality,''),'-',IFNULL(PM.City,''),', ',IFNULL(PM.Country,''))Address , ");
            sb.Append(" IF(ReceiptNo<>'','CASH','CREDIT')PaymentMode,RP.ReceiptNo,BillNo,DATE_FORMAT(RP.date,'%d-%b-%Y')DATE , ");
            sb.Append(" IFNULL(rp.AmountPaid,0)AmountPaid,CASE WHEN ( TypeOfTnx IS NULL AND RP.TransactionID = '' ) THEN 'OPD Advance/Settlement' WHEN (RP.LedgerNoDr='HOSP0005') THEN 'OPDSettlement' ");
          ...
          //  sb.Append(" FROM ( SELECT rt.ReceiptNo, ");
            sb.Append(" FROM ( SELECT pmh.Type,pnl.PanelID,pnl.Company_Name AS PanelName,rt.ReceiptNo, ");
            ...
            if (rblCon.SelectedValue == "1")
                sb.Append(" INNER JOIN f_ledgertransaction lt ON rt.AsainstLedgerTnxNo = lt.LedgerTransactionNo ");
            else
                sb.Append(" LEFT JOIN f_ledgertransaction lt ON rt.AsainstLedgerTnxNo = lt.LedgerTransactionNo AND lt.Iscancel=0 ");

            sb.Append(" INNER JOIN `patient_medical_history` pmh ON pmh.TransactionID=lt.TransactionID INNER JOIN `f_panel_master` pnl ON pnl.PanelID=pmh.PanelID ");
            ...
            if (rblCon.SelectedValue == "1")
            {
                //for credit only
                sb.Append(" UNION ALL ");
                // sb.Append(" SELECT ''ReceiptNo,lt.NetAmount ,lt.TypeOfTnx,lt.TransactionID,'' AsainstLedgerTnxNo,lt.LedgerTransactionNo,lt.BillNo,lt.Date,lt.time ");
                sb.Append(" SELECT pmh.Type, pnl.PanelID,pnl.Company_Name AS PanelName,''ReceiptNo,lt.NetAmount ,lt.TypeOfTnx,lt.TransactionID,'' AsainstLedgerTnxNo,lt.LedgerTransactionNo,lt.BillNo,lt.Date,lt.time ");
               
                
                sb.Append(",lt.LedgerNoCr,lt.PatientID,lt.PaymentModeID,'' LedgerNoDr FROM f_ledgertransaction lt ");
               // sb.Append(" WHERE TypeOfTnx NOT IN ('Pharmacy-Issue','Pharmacy-Return') AND lt.IsCancel=0 AND ( lt.PaymentModeID=4 OR lt.Adjustment=0 ) AND lt.CentreID='" + Session["CentreID"].ToString() + "' ");

                sb.Append(" INNER JOIN `patient_medical_history` pmh ON pmh.TransactionID=lt.TransactionID INNER JOIN `f_panel_master` pnl ON pnl.PanelID=pmh.PanelID ");
                
                sb.Append(" WHERE  lt.IsCancel=0 AND ( lt.PaymentModeID=4 OR lt.Adjustment=0 ) AND lt.CentreID='" + Session["CentreID"].ToString() + "' ");
                ...
                if (BillNo != "")
                {
				 ...
				 sb.Append(" AND IFNULL(LT.BillNo,'')<>'' GROUP BY lt.LedgerTransactionNo");
            }
            ...
            sb.Append(" )a  ");
           // sb.Append(" )a WHERE TypeOfTnx NOT IN ('Pharmacy-Issue','Pharmacy-Return') ");
    ...
    protected void btnView_Click(object sender, EventArgs e)
    {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            GridView1.FooterRow.Cells[8].Text = total.ToString("N2");
            GridView1.FooterRow.Cells[8].HorizontalAlign = HorizontalAlign.Right;
            btnPrintCombined.Visible = true;
        }
        else
        {
            pnlHide.Visible = false;
            btnPrintCombined.Visible = false;
    ...
    protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (lblTypeofTnx.Text.ToUpper() != "IPD")
        {
            ...
            else
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "key", "window.open('../../Design/Common/CommonPrinterOPDThermal.aspx?ReceiptNo=" + receiptNo + "&LedgerTransactionNo=" + dtLedTno + "&IsBill=" + IsBill + "&Duplicate=" + ChkOriginalPrint + "&Type=OPD');", true);
                   // ScriptManager.RegisterStartupScript(this, this.GetType(), "key", "window.open('../../Design/Common/CommonPrinterOPDThermal.aspx?LedgerTransactionNo='" + dtLedTno + "'&IsBill='" + IsBill + "' &Duplicate=0&Type=OPD');", true);
                }
            }
        }
        if (lblTypeofTnx.Text.ToUpper() == "IPD")
        {
            ...
            else
            {
                ...
                //ScriptManager.RegisterStartupScript(this, this.GetType(), "key", "window.open('../../Design/Common/CommonReceipt_pdf.aspx?ReceiptNo=" + receiptNo + "&Duplicate=" + ChkOriginalPrint + "&Type=IPD');", true);
                ScriptManager.RegisterStartupScript(this, this.GetType(), "key", "window.open('../../Design/Common/CommonPrinterOPDThermal.aspx?ReceiptNo=" + receiptNo + "&LedgerTransactionNo=" + dtLedTno + "&IsBill=" + IsBill + "&Duplicate=" + ChkOriginalPrint + "&Type=IPD');", true);
            }
        }
    }
    ...
    protected void btnPrintCombined_Click(object sender, EventArgs e)
    {
        lblMsg.Text = "";
        string PreviousCombineId = "";
        string LedgerTransactionNo = "";
        string NewCombineId = "";
        foreach (GridViewRow row in GridView1.Rows)
        {
            if (((CheckBox)row.FindControl("chkCombinedPrint")).Checked)
            {
                ////if (PreviousCombineId == "")
                ////    PreviousCombineId = ((Label)row.FindControl("lblCombinedID")).Text;


                ////NewCombineId = ((Label)row.FindControl("lblCombinedID")).Text;

                ////if (PreviousCombineId != NewCombineId)
                ////{
                ////    lblMsg.Text = "Multi Patients OR Multi Covered By Bills are not Allowed in a Combined Bill.";
                ////    return;
                ////}


                if (LedgerTransactionNo == "")
                    LedgerTransactionNo = ((Label)row.FindControl("lblLedgerTransactionNo")).Text;
                else
                    LedgerTransactionNo = LedgerTransactionNo + "," + ((Label)row.FindControl("lblLedgerTransactionNo")).Text;
        ...
        if (LedgerTransactionNo == "")
        {
            lblMsg.Text = "Please Select Atleast One.";
              return;
        }

        ScriptManager.RegisterStartupScript(this, this.GetType(), "key12121", "window.open('../../Design/Common/OPDCombinedBill.aspx?LedgerTransactionNo=" + LedgerTransactionNo + "&Type=Combined');", true);
