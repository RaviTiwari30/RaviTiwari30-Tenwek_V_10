...
public partial class Design_OPD_OPDFinalSettlement : System.Web.UI.Page
{
    ...
    private void BindPanel()
    {
        ...
        ddlPanelCompany.Items.Insert(0, "Select");
    }

    protected void btnView_Click(object sender, EventArgs e)
    {
        string dptlno = Session["DeptLedgerNo"].ToString();
        if (ddlPanelCompany.SelectedItem.Value == "0" && txtPatientID.Text.Trim() == "")
        {
            lblMsg.Text = "Please Select Panel or Enter UHID";
            txtPatientID.Focus();
            return;
        }
        parentClear();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (ValidateValues())
        {
            ...
            try
            {
                ...
                if (Util.GetDecimal(txtAmount.Text.Trim()) == 0)
                {
                    lblMsg.Visible = true;
                ...
                {
                    if (Util.GetDecimal(txtAmount.Text.Trim()) > Util.GetDecimal(lblAmtSum.Text.Trim()))
                    {
                        lblMsg.Visible = true;
                ...
                if (ViewState["PaidType"].ToString() == "Credit") // CREDIT  CASES
                {
                    ...
                    OPD opd = new OPD();
                    receiptNo = opd.SaveCreditReceipt(payment, Util.GetInt(lblPanelID1.Text), amount, lblLedgertransactionNo.Text, lblMRNo.Text, lblTransactionID.Text, paidBy, hashCode, amountPaid, roundOff, tnx, lblPaidBy.Text, con, Util.GetDecimal(lblOPDAdvance.Text), lblDeptLedgerNo.Text.Trim());
                    ...
                    if (receiptNo != "")
                    {
                        ...
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "key2", "location.href='OPDFinalSettlement.aspx';", true);
                ...
                else
                {
                    ...
                    OPD opd = new OPD();
                    receiptNo = opd.SavePendingReceipt(payment, Util.GetInt(lblPanelID1.Text), amount, lblLedgertransactionNo.Text, lblMRNo.Text, lblTransactionID.Text, paidBy, txtNarration.Text.Trim(), type, hashCode, amountPaid, roundOff, tnx, lblPaidBy.Text, con, Util.GetDecimal(lblOPDAdvance.Text),lblDeptLedgerNo.Text.Trim());
                    if (receiptNo != "")
                    {
                        tnx.Commit();
                        Clear();
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "key", "alert('Receipt Saved Successfully');window.open('../../Design/common/ReceiptOPD.aspx?ReceiptNo=" + receiptNo + "&balance=" + ViewState["balance"].ToString() + "&TransactionID=" + lblTransactionID.Text + "&Duplicate=0');", true);
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "key1", "location.href='OPDFinalSettlement.aspx';", true);
