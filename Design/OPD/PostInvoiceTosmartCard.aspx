<%@ Page Title="" Language="C#" MasterPageFile="~/DefaultHome.master" AutoEventWireup="true" CodeFile="PostInvoiceTosmartCard.aspx.cs" Inherits="Design_OPD_PostInvoiceTosmartCard" %>
   ...
   <script type="text/javascript">
	...
	<div id="Pbody_box_inventory">
		...
		<div class="POuter_Box_Inventory" style="text-align: center;">
				...
				<b>Claim Submission<asp:Label ID="lblMaxDispatchNO" runat="server"></asp:Label></b><br />
			<span id="spnErrorMsg" class="ItDoseLblError"></span>

            
            <label id="checkedPoolNr" style="display:none"></label>
				...
				<div class="col-md-1"></div>
				 <div class="col-md-22">
					 ...
					 <div class="row">
						...
						<div class="col-md-5">
                              <asp:TextBox ID="txtEncounterNo" runat="server" TabIndex="5"  MaxLength="10" ClientIDMode="Static"></asp:TextBox>
						   <asp:TextBox ID="txtIPDNo" runat="server" TabIndex="5"  MaxLength="10" ClientIDMode="Static" Visible="false"></asp:TextBox>
						</div>
						 <div class="col-md-3">
							<label class="pull-left">
							 Docket No.
	...
	<script type="text/javascript">
	    ...
	    function bindData() {
	        ...
	        if ($("#txtEncounterNo").val() == "" || $("#txtEncounterNo").val() == null || $("#txtEncounterNo").val() == undefined) {
	            modelAlert("Please Enter Encounter No", function () {
	            });
	            return;
	        }
	        ...
	        $.ajax({
	            type: "POST",
	            url: "PostInvoiceTosmartCard.aspx/GetMessages",
	            data: '{BillFromDate:"' + $("#ucFromDate").val() + '",BillToDate:"' + $("#ucToDate").val() + '", PatientID:"' + $("#txtMRNo").val() + '", ClaimNo:"' + $("#txtClaimNo").val() + '", PolicyNo:"' + $("#txtPolicyNo").val() + '", IPDNo:"", DocketNo:"' + $("#txtDocketNo").val() + '", DispatchNo:"' + $("#txtDisptchNo").val() + '", DispatchDate:"' + $("#txtDisptachDate").val() + '", PanelID:"' + $("#ddlPanelCompany").val().split('$')[0] + '",Type:"' + $("#rdoOPDIPDList input[type=radio]:checked").val() + '", Status:"' + $("#rdbStatus input[type=radio]:checked").val() + '", CashCredit:"' + $("#rbtCashCredit input[type=radio]:checked").val() + '", chkDispatchDate:"' + $("#chkDisDate").is(':checked') + '",IsCoverNote:"' + $("#ddlPanelCompany").val().split('$')[1] + '",EncounterNo:"' + $("#txtEncounterNo").val() + '"}',
	            contentType: "application/json; charset=utf-8",
	            dataType: "json",
	            success: function (result) {
	                ...
	                if (DispatchData != null) {
	                    ...
	                    $("#spnPanel,#spnType,#spnInvoiceNo").text('');
	...
	style="width:200%; border-collapse:collapse;">		
		...
		#>            
		   ...
		   <tr id="Header">            
			<th class="GridViewHeaderStyle" scope="col" style="width:70px;">
               ...
               <th class="GridViewHeaderStyle" scope="col" style="width:200px;">PoolNr</th>
			<th class="GridViewHeaderStyle" scope="col" style="width:200px;">PoolDesc</th>
	   ...
	   <#}  #>
					...
					<td class="GridViewLabItemStyle" id="tdSNo">
						...
						<#=j+1#></td>
                         <td class="GridViewLabItemStyle" id="tdPoolNr" style="width:80px;text-align:right"><#=objRow.PoolNr#></td> 
					<td class="GridViewLabItemStyle" id="tdPoolDesc" style="width:150px;text-align:right"><#=objRow.PoolDesc#></td> 
					...
					<td class="GridViewLabItemStyle" id="tdDocket" style="width:140px;">
                          <input type="text" id="txtDocket"  class="docket" style="width:80px" value="<#=objRow.EncounterNo#>"/>
	...
	<script type="text/javascript">
	    ...
	    function chkSelect(rowID) {
	        ...
	        var IsChecked = $(rowID).closest('tr').find('#chkSelectSNo').is(":checked");
	        var val = $(rowID).closest('tr').find('#tdPoolNr').text();


	        if (IsChecked) {

	            if ($("#checkedPoolNr").text() == "") {
	                $("#checkedPoolNr").text(val);
	            } else {
	                if ($("#checkedPoolNr").text() != val) {
	                    modelAlert(" Check bill  of Same PoolNr.");
	                    $(rowID).closest('tr').find('#chkSelectSNo').prop("checked", false);

	                } else {
	                    $(rowID).closest('tr').find('#chkSelectSNo').prop("checked", true);
	                }
	            }
	            ...
	            $('#tb_grdSearch').find("#chkSelectSNo").each(function () {
	...
	<script type="text/javascript">
	    function IPDInvoice(InvoiceNo) {
	        $.ajax({
	            url: "PostInvoiceTosmartCard.aspx/IPDInvoice",
	                    ...
	                    success: function (result) {
	                        if (result.d != "0") {
	                            ...
	                            if (result.d != "") {
	                                ...
	                                serverCall('Services/CardIntigration.asmx/SubmitClaim', { PanelId: $("#ddlPanelCompany").val().split('$')[0], InvoiceNo: result.d, EncounterNo: $("#txtEncounterNo").val() }, function (response) {
	                                    ...
	                                    if ($responseData.status) {
	                                        ...
	                                        if ($responseData.Msg.message == null || $responseData.Msg.message == "" || $responseData.Msg.message == undefined) {
	                                            debugger;
	                                            modelAlert("Internal Server Error,Smartcard side.", function () {
	                                                $("#btnSave").val('Submit Claim').removeAttr('disabled');
	                                                bindData();
	                                            })
	                                        } else {
	...
	<script type="text/javascript">
	    ...
	    function ReSubmitClaim() {
	        ...
	        serverCall('Services/CardIntigration.asmx/SubmitClaim', { PanelId: $("#ddlPanelCompany").val().split('$')[0], InvoiceNo: InvoiceNo, EncounterNo: $("#txtEncounterNo").val() }, function (response) {
	            var $responseData = JSON.parse(response);
	            console.log($responseData);
