<%@ Page Language="C#" AutoEventWireup="true" CodeFile="DisplayRadiologyToken.aspx.cs" Inherits="Design_Token_DisplayRadiologyToken" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function LoadTokenList() {
            var Id = "";
            var DptID = "";
            var DepartmentID = '<%=Util.GetString(Request.QueryString["docDepartmentID"])%>';
            if (DepartmentID != null && DepartmentID != "") {
                var length = DepartmentID.split(',').length;
                if (length > 1) {
                    for (var i = 0; i < DepartmentID.split(',').length; i++) {
                        id = DepartmentID.split(',')[i];
                        DptID += "'" + id + "',";
                    }
                    DepartmentID = DptID.slice(0, -1);
                }
                else {
            ...
            $.ajax({
                url: "DisplayRadiologyToken.aspx/DisplayTokenList",
        ...
        <div>
         ...
         <script id="tb_PatientLabSearch" type="text/html">
                        <table id="tbl_Doctors" class="tbPatient" border="0">
                         ...
                         #>                       
                        <tr id="td_<#=objRow.DeptId#>"<#if(j==0){#>class="SelectedDoctor"<#} else
                             {#> 
                                  <# if(Doctor_Data[j].DeptId==Doctor_Data[j-1].DeptId){ #> class="Doctor"
                            <#}else{ #>
                            ...
                            class="Doctor"
                             <#}#>
                            >
                         <td  style="width:25%;text-align:left;"><#= objRow.DepartmentName.toProperCase() #></td>
                         <td style="width:25%;text-align:left;"><#=objRow.InvestigationName.toProperCase() #></td>
                         <td style="width:25%;text-align:left;"><#=objRow.pname.toProperCase() #></td>
                        <%-- <# if(j!=0){#>
                           ...
                           <%-- <td style="text-align:center;" id="blink">
                            ...
                            <#} else
                             {#> 
                              <# if(Doctor_Data[j].DeptId==Doctor_Data[j-1].DeptId){ #> 
                          <td style="text-align:center;"><#=objRow.TokenNo.toProperCase() #></td>
                            <#}else{ #>
