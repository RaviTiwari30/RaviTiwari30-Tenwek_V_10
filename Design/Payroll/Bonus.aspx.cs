...
public partial class Design_Payroll_Bonus : System.Web.UI.Page
{
    private int a = 0;
    ...
    {
        lblmsg.Text = "";
        decimal AccomodationAmt = Util.GetDecimal(txtBonusAmt.Text);
        ...
        try
        {
            ...
            str = "delete from pay_empsalary_detail where MasterID=" + MasterID + " and TypeID IN(40,50)";
            ...
            {
                str = "insert into pay_empsalary_detail (MasterID,EmployeeID,TypeID,TypeName,Amount,RemunerationType,UserID,TransactionType)values('" + MasterID + "','" + lblEmpID.Text.Trim() + "','40','" + lblTypeName.Text.Trim() + "'," + AccomodationAmt + ",'E','" + ViewState["UserID"].ToString() + "','7')";
                a = MySqlHelper.ExecuteNonQuery(Tranx, CommandType.Text, str);
                if (a == 0)
                {
                    Tranx.Rollback();
                ...
                str = "insert into pay_empsalary_detail (MasterID,EmployeeID,TypeID,TypeName,Amount,RemunerationType,UserID,TransactionType)values('" + MasterID + "','" + lblEmpID.Text.Trim() + "','50','Bonus TAX'," + AccomodationTaxAmt + ",'D','" + ViewState["UserID"].ToString() + "','7')";
                a = MySqlHelper.ExecuteNonQuery(Tranx, CommandType.Text, str);
                if (a == 0)
                {
                    Tranx.Rollback();
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            EmpGrid.DataBind();
            lblTypeName.Text = StockReports.ExecuteScalar("select Name from pay_remuneration_master where ID=40").ToString();
        }
        else
        {
    ...
    protected void EmpGrid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Select")
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                sb.Append("  INNER JOIN pay_empsalary_detail det ON mst.ID=det.MasterID AND Amount>0 AND det.TypeID IN(40,50) AND ");
                ...
                {
                    txtBonusAmt.Text = Accomodation.Rows[0]["Amount"].ToString();
                    txtBonusTaxAmt.Text = Accomodation.Rows[1]["Amount"].ToString();
                }
                else
                {
                    txtBonusAmt.Text = "0";
                    txtBonusTaxAmt.Text = "0";
                }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            txtBonusAmt.Attributes.Add("onKeyUp", "CalBonus()");
            BindDate();
