<%@ Page Language="C#" AutoEventWireup="true" CodeFile="DispatchMaster.aspx.cs" Inherits="Design_Dispatch_DispatchMaster"
   ...
   <script type="text/javascript">
	...
	<div id="Pbody_box_inventory">
		...
		<div class="POuter_Box_Inventory" style="text-align: center;">
				...
				<b>Generate Invoice<asp:Label ID="lblMaxDispatchNO" runat="server"></asp:Label></b><br />
					 ...
					 <div class="row">
						...
						<div class="col-md-5">
						  <asp:RadioButtonList ID="rdoOPDIPDList" onclick="chkOPDIPDList()" TabIndex="14" ClientIDMode="Static" runat="server"  RepeatDirection="Horizontal" >
                            ...
                            <asp:ListItem Text="Emergency" Value="EMG"></asp:ListItem>
		...
		<div class="POuter_Box_Inventory" style="text-align: center;" >
			...
			<tr>
				...
				<td style="width: 50%;text-align:left">                   
					<input type="button" value="Save" id="btnSave" class="ItDoseButton" onclick="saveDispatch()" />
				</td>
			</tr>
		</table>
		</div>
	...
	<script type="text/javascript">
	    ...
	    function bindData() {
	        ...
	        $("#spnIPDNo").text($("#txtIPDNo").val());

	        $("#btnSearch").val("Searching...").attr('disabled', 'disabled');
	       // $.blockUI({ message: 'Please Wait.....\n<img src="../../Images/loadingAnim.gif" />' });
	        $.ajax({
	            type: "POST",
	            url: "DispatchMaster.aspx/GetMessages",
	            data: '{BillFromDate:"' + $("#ucFromDate").val() + '",BillToDate:"' + $("#ucToDate").val() + '", PatientID:"' + $("#txtMRNo").val() + '", ClaimNo:"' + $("#txtClaimNo").val() + '", PolicyNo:"' + $("#txtPolicyNo").val() + '", IPDNo:"' + $("#txtIPDNo").val() + '", DocketNo:"' + $("#txtDocketNo").val() + '", DispatchNo:"' + $("#txtDisptchNo").val() + '", DispatchDate:"' + $("#txtDisptachDate").val() + '", PanelID:"' + $("#ddlPanelCompany").val().split('$')[0] + '",Type:"' + $("#rdoOPDIPDList input[type=radio]:checked").val() + '", Status:"' + $("#rdbStatus input[type=radio]:checked").val() + '", CashCredit:"' + $("#rbtCashCredit input[type=radio]:checked").val() + '", chkDispatchDate:"' + $("#chkDisDate").is(':checked') + '",IsCoverNote:"' + $("#ddlPanelCompany").val().split('$')[1] + '"}',
	            contentType: "application/json; charset=utf-8",
	            dataType: "json",
	...
	style="width:200%; border-collapse:collapse;">		
		...
		#>            
			...
			{#>
		   <tr id="Header">            
			<th class="GridViewHeaderStyle" scope="col" style="width:70px;"><input type="checkbox" class="chkAll"  onclick="chkAll(this)" />S.No.</th>
	   ...
	   <#}  #>
					...
					<td class="GridViewLabItemStyle" id="tdDocket" style="width:140px;">
							<input type="text" id="txtDocket"  class="docket" style="width:80px" value="<#=objRow.DocketNo#>"/><span style="color: red; font-size: 10px;" class="shat">*</span>
					<span id="getDocketNo" style="display:none"   <# if($.trim(DispatchData[j].DocketNo) !="")
						...
						<td class="GridViewLabItemStyle" id="tdPanelInvoiceNo"  style="width:80px;display:none"><#=objRow.panelinvoiceno#></td> 
					</tr>
		<#}
		#>       
	 </table>
	...
	<script type="text/javascript">
	    function IPDInvoice(InvoiceNo) {
	        $.ajax({
	            url: "DispatchMaster.aspx/IPDInvoice",
	        ...
	        if ($(".chkSelect:checked").length >= 1) {
	            ...
	            if ((dispatchData().length > 0) && (DocketCon == "0")) {
	               // $.blockUI({ message: 'Please Wait.....\n<img src="../../Images/loadingAnim.gif" />' });
	                ...
	                $.ajax({
	                    url: "Services/Panel_Invoice.asmx/saveDispatch",
	                    ...
	                    success: function (result) {
	                        if (result.d != "0") {
	                            ...
	                            if (result.d != "") {
	                                modelAlert("Record Saved Successfully. </br> Invoice No. : <span class=patientInfo>" + result.d + "</span>", function () {
	                                 //   dispatchReport(result.d);
	                                    $("#btnSave").val('Save').removeAttr('disabled');
	                                    bindData();
	                                });
	                                // if ($("#rdoOPDIPDList input[type=radio]:checked").val() == "OPD")
	                                //     dispatchReportOPD(result.d);
	                                // else {
	                                //     IPDInvoice(result.d);
	                                // $find('mpDispatchIPD').show();
	                                // }
	                                //
	                            }
	                            else {
