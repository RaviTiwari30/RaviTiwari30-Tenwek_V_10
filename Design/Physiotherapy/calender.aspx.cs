...
public partial class Design_Physiotherapy_Calender : System.Web.UI.Page
{
    ...
    public static IList GetEvents(string DocID)
    {
        ...
        try
        {
            ...
            string query = "CALL physio_calendar('" + DocID + "')";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                events.Add(new Event
                {
                    ...
                    EventEndtime = Util.GetDateTime(dt.Rows[i]["EndTime"].ToString()).AddMinutes(10).ToString("H:mm"),
                    ImageType = dt.Rows[i]["ISConform"].ToString(),
                    tooltip = "<p  id='tooltip' style='top:152px; left:128px; position:absolute;background-color:#C2E6FF'><strong>Diagnosis</strong> : " + dt.Rows[i]["icd_desc"].ToString() + "</p>",
