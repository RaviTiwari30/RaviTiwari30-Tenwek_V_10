...
public class Patient_Lab_Radiology_OPD
{
    ...
    public Patient_Lab_Radiology_OPD()
    {
        objCon = Util.GetMySqlCon();
        this.IsLocalConn = true;
    }

    public Patient_Lab_Radiology_OPD(MySqlTransaction objTrans)
    {
    ...
    public int Insert()
    {
        try
        {
            ...
            objSQL.Append("INSERT INTO patient_labRadiology_OPD(Investigation_ID,Test_ID, Result_Date, Result_Time, Template_Desc )");
            objSQL.Append("VALUES (@Investigation_ID, @Test_ID, @Result_Date, @Result_Time, @Template_Desc)");
    ...
    public int Update()
    {
        try
        {
            int RowAffected;
            StringBuilder objSQL = new StringBuilder();
            objSQL.Append("UPDATE patient_labRadiology_OPD SET  Investigation_ID= ?, Result_Date=?, Result_Time=?, Template_Desc = ?,Test_ID=? WHERE ID = ? ");
    ...
    public int Delete()
    {
        try
        {
            int iRetValue;
            StringBuilder objSQL = new StringBuilder();
            objSQL.Append("DELETE FROM patient_labRadiology_OPD WHERE ID = ?");
    ...
    public bool Load()
    {
        ...
        try
        {
            string sSQL = "SELECT * FROM patient_labRadiology_OPD WHERE ID = ?";
    ...
    {
        this.Investigation_ID = Util.GetString(dtTemp.Rows[0][AllTables.patient_labRadiology_opd.Investigation_ID]);
        this.Result_Date = Util.GetDateTime(dtTemp.Rows[0][AllTables.patient_labRadiology_opd.Result_Date]);
        this.Result_Time = Util.GetDateTime(dtTemp.Rows[0][AllTables.patient_labRadiology_opd.Result_Time]);
        this.Template_Desc = Util.GetString(dtTemp.Rows[0][AllTables.patient_labRadiology_opd.Template_Desc]);
        this.Test_ID = Util.GetString(dtTemp.Rows[0][AllTables.patient_labRadiology_opd.Test_ID]);
    }
