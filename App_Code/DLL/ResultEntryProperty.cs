using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

/// <summary>
/// Summary description for Patient_lab_ObservationOPD
/// </summary>
public class ResultEntryProperty
{
    public ResultEntryProperty()
    {
        //
        // TODO: Add constructor logic here
        //
    }
    public string ID { get; set; }
    public string INV_PrintSeparate { get; set; }
    public string OBS_PrintSeparate { get; set; }
    public string LabInvestigation_ID { get; set; }
    public string Investigation_ID { get; set; }
    public string PLIID { get; set; }
    public string Result_Flag { get; set; }
    public string isHold { get; set; }
    public string machinename { get; set; }
    public string MachineID1 { get; set; }
    public string MachineID2 { get; set; }
    public string MachineID3 { get; set; }
    public string DefaultValue { get; set; }
    public string Reading1 { get; set; }
    public string Reading2 { get; set; }
    public string Reading3 { get; set; }
    public string Investigation_ID1 { get; set; }
    public string DefaultReading { get; set; }
    public string rangetype { get; set; }
    public string ReportType { get; set; }
    public string ParentID { get; set; }
    public string Child_Flag { get; set; }
    public string Formula { get; set; }
    public string Print_Sequence { get; set; }
    public string IsMultiChild { get; set; }
    public string Help { get; set; }
    public string IsComment { get; set; }
    public string ResultRequired { get; set; }
    public string isPartial_Result { get; set; }
    public string DLCCheck { get; set; }
    public string Gender { get; set; }
    public string AgeInDays { get; set; }
    public string Inv { get; set; }
    public string Isreflex { get; set; }
    public string isAmr { get; set; }
    public string AmrMin { get; set; }
    public string AmrMax { get; set; }
    public string AutoMin { get; set; }
    public string AutoMax { get; set; }
    public string ReflexMin { get; set; }
    public string ReflexMax { get; set; }
    public string HoldObsId { get; set; }
    public string BarcodeNo { get; set; }
    public string TestName { get; set; }
    public string IsPackage { get; set; }
    public string ShowAbnormalAlert { get; set; }
    public string ShowDeltaReport { get; set; }

    public string RequiredField { get; set; }
    public string SubCategoryID { get; set; }
    public string Value1 { get; set; }
    public string ManualValue { get; set; }
    public string LabNo
    {
        get;
        set;
    }
    public string LabObservation_ID
    {
        get;
        set;
    }

    public string Test_ID
    {
        get;
        set;
    }

    public string Result_Date
    {
        get;
        set;
    }

    public string Result_Time
    {
        get;
        set;
    }

    public string Value
    {
        get;
        set;
    }

    public string Description
    {
        get;
        set;
    }

    public string MinValue
    {
        get;
        set;
    }

    public string MaxValue
    {
        get;
        set;
    }

    public string LabObservationName
    {
        get;
        set;
    }

    public string Priorty
    {
        get;
        set;
    }

    public string InvPriorty
    {
        get;
        set;
    }

    public string ReadingFormat
    {
        get;
        set;
    }

    public string DisplayReading
    {
        get;
        set;
    }

    public string OrganismID
    {
        get;
        set;
    }

    public string IsOrganism
    {
        get;
        set;
    }

    public string ParamEnteredBy
    {
        get;
        set;
    }

    public string ParamEnteredByName
    {
        get;
        set;
    }

    public string MacReading
    {
        get;
        set;
    }

    public string dtMacEntry
    {
        get;
        set;
    }

    public string MachineID
    {
        get;
        set;
    }

    public string Method
    {
        get;
        set;
    }

    public string PrintMethod
    {
        get;
        set;
    }

    public string LedgerTransactionNo
    {
        get;
        set;
    }

    public string IsCommentField
    {
        get;
        set;
    }

    public string ResultRequiredField
    {
        get;
        set;
    }

    public string IsCritical
    {
        get;
        set;
    }

    public string MinCritical
    {
        get;
        set;
    }

    public string MaxCritical
    {
        get;
        set;
    }

    public string Approved
    {
        get;
        set;
    }

    public string ApprovedBy
    {
        get;
        set;
    }

    public string ApprovedDate
    {
        get;
        set;
    }

    public string ApprovedName
    {
        get;
        set;
    }

    public string ResultEnteredBy
    {
        get;
        set;
    }

    public string ResultEnteredDate
    {
        get;
        set;
    }

    public string ResultEnteredName
    {
        get;
        set;
    }

    public string AbnormalValue
    {
        get;
        set;
    }

    public string IsBold
    {
        get;
        set;
    }

    public string IsUnderLine
    {
        get;
        set;
    }

    public string isMic
    {
        get;
        set;
    }

    public string PrintSeparate
    {
        get;
        set;
    }

    public string UpdateID
    {
        get;
        set;
    }

    public string UpdateName
    {
        get;
        set;
    }

    public string Updatedate
    {
        get;
        set;
    }

    public string Flag
    {
        get;
        set;
    }

    public string isrerun
    {
        get;
        set;
    }
    public string pliIsReRun
    {
        get;
        set;
    }
    public string SaveInv
    {
        get;
        set;
    }
}