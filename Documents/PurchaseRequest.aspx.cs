...
public partial class Design_Purchase_PurchaseRequest : System.Web.UI.Page
{
            ...
            else
            {
                ...
                else
                {
                    if (Session["DeptLedgerNo"] != null)
                    {
                        ...
                        hdnDeptledgerNo.Value = ViewState["DeptLedgerNo"].ToString();
                    }
    ...
    [WebMethod(EnableSession = true)]
    public static string SaveAllDetails(string chkprnt, string StoreId, string Narration, string RequestType, string DepartMent, string ledgerNo)
    {
        string PRNo = SaveRequest(StoreId, Narration, RequestType, DepartMent, ledgerNo);
        if (PRNo != string.Empty)
        {
            if (chkprnt == "True")
            {
    ...
    [WebMethod(EnableSession = true)]
    public static string SaveRequest(string StoreId, string Narration, string RequestType, string DepartMent, string ledgerNo)
    {
        string typeOfTnx = "";
        ...
        if (HttpContext.Current.Session["dtItems"] != null)
        {
            ...
            if (dtItems.Rows.Count > 0)
            {
                ...
                try
                {
                   ...
                   // PRMst.IssuedTo = HttpContext.Current.Session["DeptLedgerNo"].ToString();
                    ...
                    if (PRNo != string.Empty)
                    {
                        foreach (DataRow dr in dtItems.Rows)
                        {
                            ...
                            str = "Call Insert_PurchaseRequestDetail('" + PRNo + "','" + Util.GetString(dr["ItemID"]) + "','" + Util.GetString(dr["ItemName"]).Replace("'", "''") + "','" + Util.GetString(dr["SubCategory"]) + "'," + Util.GetFloat(dr["RequiredQty"]) + "," + Util.GetFloat(dr["RequiredQty"]) + ",0,'" + Util.GetString(dr["Specification"]) + "','" + Util.GetString(dr["Purpose"]) + "',0,'" + Util.GetString(dr["Unit"]) + "','" + Util.GetString(dr["Dept"]) + "','" + typeOfTnx + "','" + storeLedgerNo + "','" + HttpContext.Current.Session["HOSPID"].ToString() + "','" + Util.GetInt(HttpContext.Current.Session["CentreID"].ToString()) + "');";
